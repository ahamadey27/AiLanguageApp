...
function playSoundSequence(soundParamsArray) {
    ...
    soundParamsArray.forEach(params => {
        if (params.Frequency === 0 && params.Duration > 0) { // Handle silence
            currentTime += (params.Duration / 4) / 1000; // Add silence duration to current time, quartered
        ...
        gainNode.gain.linearRampToValueAtTime(0, currentTime + ((params.Duration / 4) / 1000) - 0.01); // Ramp down just before stopping
        ...
        oscillator.stop(currentTime + (params.Duration / 4) / 1000); // Convert duration from ms to seconds

        // Advance current time for the next sound
        currentTime += (params.Duration / 4) / 1000;
    });
